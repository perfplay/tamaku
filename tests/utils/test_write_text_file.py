...
class TestWriteFileFunctions(unittest.TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch.object(logger.logger, 'info')
    def test_write_text_file_success(self, mock_logger_info, mock_file):
        data = "This is a test"
        ...
        mock_logger_info.assert_called_once_with(f"Generated file: {filename}")

    @patch("builtins.open", new_callable=mock_open)
    @patch.object(logger.logger, 'error')
    def test_write_text_file_ioerror(self, mock_logger_error, mock_file):
        data = "This is a test"
        filename = "dummy_path.txt"
        ...
        mock_file.assert_called_once_with(filename, 'w')
        mock_logger_error.assert_called_once_with(f"Failed to write file: File write error")
